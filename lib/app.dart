...
class _FruitScreenState extends State<FruitScreen> {
  ...
  final List<Map<String, dynamic>> fallbackFruits = [
    {
      "name": "Persimmon",
      "id": 52,
      "family": "Ebenaceae",
    ...
    if (allFruits == null) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            allFruits = fallbackFruits;
          });
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                'Fehler beim Abrufen der Daten: ${response.statusCode}, Fallback-Daten werden verwendet.'),
          ));
        }
      } catch (e) {
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(
              'Ein Fehler ist aufgetreten: $e, Fallback-Daten werden verwendet.'),
        ));
      } finally {
        setState(() {
          isLoading = false;
  ...
  void _findFruit(String name) {
    if (allFruits != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Früchte-Daten sind nicht geladen.'),
      ));
    }
  }
            ...
            ElevatedButton(
              onPressed: () {
                if (_controller.text.isNotEmpty) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text('Bitte gib einen Fruchtnamen ein.'),
                  ));
                }
              },
                        ...
                        child: ListView(
                          children: [
                            ListTile(
                              ...
                              subtitle: Text(fruitData!['name']),
                            ),
                            ListTile(
                              title: const Text('Family'),
                              subtitle: Text(fruitData!['family']),
                            ),
                            ListTile(
                              title: const Text('Genus'),
                              subtitle: Text(fruitData!['genus']),
                            ),
                            ListTile(
                              title: const Text('Order'),
                              subtitle: Text(fruitData!['order']),
                            ),
                            ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  Text(
                                      'Kalorien: ${fruitData!['nutritions']['calories']}'),
                                  Text(
                                      'Zucker: ${fruitData!['nutritions']['sugar']}'),
                                  Text(
                                      'Fett: ${fruitData!['nutritions']['fat']}'),
                                  Text(
                                      'Proteine: ${fruitData!['nutritions']['protein']}'),
                                  Text(
                                      'Kohlenhydrate: ${fruitData!['nutritions']['carbohydrates']}'),
                                ],
                              ),
                            ),
                          ],
